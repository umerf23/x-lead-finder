...
What this adds over build_dashboard.py:
  ...
  - A Describe tab that turns a plain English sentence into a search
    query, previews it against real posts, and saves it as a watchlist
...
try:
    ...
    print("")
    print("    pip install fastapi uvicorn pyyaml python-dotenv")
    print("")
    raise SystemExit(1)
...
def save_settings(settings: Settings) -> JSONResponse:
    ...
    # Read the file first, then change only the keys this page owns.
...
# ---------------------------------------------------------------------
# Turning plain English into a watchlist
# ---------------------------------------------------------------------

class WishRequest(BaseModel):
    text: str = Field(description="a plain English description")
...
def require_query_builder():
    if query_builder is None:
        raise HTTPException(
            status_code=500,
            detail="query_builder.py is missing from the project folder.")


@app.post("/api/query/draft")
def draft_query(wish: WishRequest) -> JSONResponse:
    """Ask the model to turn a sentence into a search. Costs nothing."""
    require_query_builder()
    try:
        return JSONResponse(query_builder.draft_watchlist(wish.text))
    except query_builder.QueryBuilderError as error:
        raise HTTPException(status_code=400, detail=str(error))


@app.post("/api/query/check")
def check_query(request: PreviewRequest) -> JSONResponse:
    """Look for the query shapes that reliably find nothing. Free."""
    require_query_builder()
    return JSONResponse({"warnings": query_builder.check_query(request.query)})


@app.post("/api/query/preview")
def preview_query(request: PreviewRequest) -> JSONResponse:
    """Fetch one page of real posts so you can see if it works. Spends."""
    require_query_builder()
    if not RUN_LOCK.acquire(blocking=False):
        raise HTTPException(
            status_code=409,
            detail="Something is already running. Wait for it to finish.")
    try:
        return JSONResponse(query_builder.preview_query(request.query))
    except query_builder.QueryBuilderError as error:
        raise HTTPException(status_code=400, detail=str(error))
    finally:
        RUN_LOCK.release()
...
def save_watchlist(watchlist: NewWatchlist) -> JSONResponse:
    """Add an approved watchlist to config.yaml."""
    name = watchlist.name.strip()
    query = watchlist.query.strip()
    ...
    if not query:
        raise HTTPException(status_code=400, detail="The search query is empty.")
    ...
    if any(item["name"].lower() == name.lower() for item in existing):
        raise HTTPException(
            status_code=400,
            detail=f"A watchlist called '{name}' already exists. "
    ...
    config["watchlists"] = existing
    write_config(config)

    return JSONResponse({"saved": True, "name": name,
                         "total_watchlists": len(existing)})
